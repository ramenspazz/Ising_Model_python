...
def plot2D_fit(x_data, y_data, fit_m, fit_b, num_data, errors=None,
    ...
    try:
        ...
        if errors is not None:
            (_, caps, _) = plt.errorbar(x_data, y_data, yerr=errors, fmt='.',
                                        capsize=5, elinewidth=1, Color='RED')
            for cap in caps:
                cap.set_color('RED')
...
def parse_data_file(f_name, data_cols):
    ...
    Returns a multidimensional array of containing the data from the givin file
     name in columns.
    """
    try:
...
def WLS_fit(x_data=None, y_data=None, errors=None, weight_data=None,
            xaxis_name=None, yaxis_name=None, f_name=None, data_lines=None,
            data_name=None, quiet=True):
    """
        Takes a filename string or x and y data as input. We are fitting the
        equation A_ij*x_j=b_i of the form b=C+Dx.
    ...
    try:
        ...
        if (((x_data is None and y_data is None) or f_name is None) and
           ((x_data is not None or y_data is not None) or f_name is not None)):
            ...
            if (weight_data is None):
                ...
                out_vec = np.linalg.inv((A_T_mtx @ weight_data @ A_mtx) @
                                        A_T_mtx @ weight_data @ b_vec)

            out_vec = [out_vec[0, 0], out_vec[1, 0]]
            fit_string = (f'The coefficents for the line of best fit (y=mx+c)'
                          f' are m={round_sig(out_vec[0],sig=4)},'
                          f' c={round_sig(out_vec[1],sig=4)}.')
            ...
            if not quiet:
                ...
                if (xaxis_name is None) and (yaxis_name is None):
                    plot2D_fit(x_vals, y_vals, out_vec[0][0], out_vec[1][0],
                               ln_num, data_name=data_name, errors=errors)
                else:
                    plot2D_fit(x_vals, y_vals, out_vec[0], out_vec[1], ln_num,
                               data_name=data_name, xaxis_name=xaxis_name,
                               yaxis_name=yaxis_name, errors=errors)
...
def PCA_fit(data_mtx):
    ...
    px1, py1 = (np.array([Xavg[0][0, 0], (Xavg[0]+U[0, 0]*S[0])[0, 0]]),
                np.array([Xavg[1][0, 0], (Xavg[1]+U[1, 0]*S[0])[0, 0]]))
    px2, py2 = (np.array([Xavg[0][0, 0], (Xavg[0]+U[0, 1]*S[1])[0, 0]]),
                np.array([Xavg[1][0, 0], (Xavg[1]+U[1, 1]*S[1])[0, 0]]))
...
def centered_mtx(A):
    """
        Returns a matrix where the element in each row is subtracted by its
        mean of its row.
    """
    if A.ndim <= 2:
        n = np.shape(A)[0]
...
def std_dev(data, mean, sample=True):
    """
        Returns the standard deviation of data, sample=True for sample stdev
        and sample=False for absolute stdev.
    """
    try:
        p_sum = 0
...
def ordinal_stats(sorted_vec: list | ndarray) -> tuple:
    ...
    try:
        ...
        if len(sorted_vec) % 2 == 0:
            median = (float(sorted_vec[int(len(sorted_vec)/2)-1] +
...
def covariance(x, y):
    """
        ...
        Computes the covariance inputs of x and y, also returning the Pearson r
...
def stats(data, sample=True, quiet=False):
    """
        Computes the general statistics on a set on a 1D set of data.
    ...
    try:
        ...
        for item in sorted_vec:
            # count the number of data points outside of one standard deviation
            # of the mean
            if ((item < mean - standard_deviation) or
                    (item > mean + standard_deviation)):
                n = n + 1
        if not quiet:
            print(f'The mean is {round_sig(mean,4)}\nThe sample stdev is'
                  f'{round_sig(standard_deviation,4)}.')
            print(f'The median is {round_sig(median,2)}\nThe min and max are'
                  f'{minimum} and {maximum}.')
            print(f'There are {n} items ({100*n/len(data_vec)}%) outside of'
                  f'one standard deviation of the mean.\n')
...
def fit_gaussian(data, mean, sd, n_bins=None, bin_width=None, condense=False,
                 quiet=True):
    """
    Fits a gaussian to a set of data. input, data, mean, standard deviation,
    ...
    try:
        ...
        data_min, data_max, data_len = (sorted_data[0],
                                        sorted_data[len(sorted_data)-1],
                                        len(sorted_data))
        ...
        if condense:
            ...
            edges = temp_edge
        m = modeling.models.Gaussian1D(amplitude=hist_amplitude, mean=mean,
                                       stddev=sd)
        data = m(x)

        fig, axs = plt.subplots(1, constrained_layout=True)  # noqa

        plt.hist(sorted_data, n_bins, edgecolor='black', linewidth=1.2)
        plt.plot(x, data, label=f'Gaussian Fit, $\mu={round_sig(mean,3)},'  # noqa
                                f' \sigma={round_sig(sd,3)}$')  # noqa
        plt.xlabel('Counts')
