...
# TODO: the worm algorithm.
    def metropolis(self, times: int | Int, BJ: float | Float,
        ...
        try:
            ...
            start = th.Event()
            finished = th.Event()
            thread_pool: list[th.Thread] = []
            for i in range(self.internal_arr.tc):
                thread_pool.append(th.Thread(
                    target=self.internal_arr.__Sum_Worker__,
                    args=(self.internal_arr.bounds[i],
                          res,
                          start,
                          wait,
                          finished)))
            ...
            for i in range(times):
                ...
                for node_i in rand_xy:
                    ...
                    dE_tot += dE
                # Sum all spins
                if ran_once is not True:
                    ran_once = True
                    start.set()
                else:
                    wait.clear()
                while res.qsize() != self.internal_arr.tc:
                    pass
                wait.set()
                for i in range(res.qsize()):
                    netSE_mtx[i, 0] += res.get_nowait()
                res.queue.clear()
            ...
            sys.stdout.write('Keyboard Interrupt. Exiting...\n')
            finished.set()
            wait.set()
